...
/*
	...
	The global drawFrame is rendered to window and cleared each GfxUpdate().
...
int entry(int argc, char **argv) {
	...
	for (World world = WORLD1; world < WORLD_MAX; world += 1) {
		world_images[world] = make_image_render_target(world_size.x, world_size.y, 4, 0, GetHeapAllocator());
	}
	
	draw_frame_init_reserve(&world_draw_frame, number_of_things_per_world);
	
	// Load player sprite
	Gfx_Image *player_sprite = load_image_from_disk(STR("oogabooga/examples/player.png"), GetHeapAllocator());
	...
	while (!window.should_close) {
		...
		for (World world = WORLD1; world < WORLD_MAX; world += 1) {
			...
			Vector4 clearColor = terrain_colors[world];
			Vector4 thing_color = thing_colors[world];
			
			// Reset draw frame and clear the render target
			DrawFrameReset(&world_draw_frame);
			gfx_clear_render_target(world_images[world], clearColor);
			...
			for (u64 i = 0; i < number_of_things_per_world; i += 1) {
				...
				switch (world) {
					...
					case WORLD1: 
					case WORLD3:
						DrawRectInFrame(pos, size, thing_color, &world_draw_frame); break;
					
					// In world 2 the shapes are circles
					case WORLD2: 
						DrawCircleInFrame(pos, v2(size.x, size.x), thing_color, &world_draw_frame); break;
		...
		if (IsKeyJustPressed(KEY_ARROW_LEFT))  current_world = previous_world;
		if (IsKeyJustPressed(KEY_ARROW_RIGHT)) current_world = next_world;
		...
		drawFrame.cameraXform = m4_make_translation(v3(v2_expand(camera_pos), 0.0));
		...
		if (IsKeyDown('A')) move_axes.x -= 1.0;
		if (IsKeyDown('D')) move_axes.x += 1.0;
		if (IsKeyDown('S')) move_axes.y -= 1.0;
		if (IsKeyDown('W')) move_axes.y += 1.0;
		
		move_axes = V2Normalize(move_axes);
		
		player_pos = V2Add(player_pos, V2Mulf(move_axes, 200*delta_time));
		...
		drawFrame.cameraXform = M4Identity();
		...
		for (World world = WORLD1; world < WORLD_MAX; world += 1) {
			...
			if (world == current_world) {
				...
				draw_circle(V2Sub(pos_on_minimap, v2(3, 3)), v2(6, 6), COLOR_RED);
			}
		}
