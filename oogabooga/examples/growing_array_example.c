...
int entry(int argc, char **argv) {
    ...
    Circle *circles;
    growing_array_init((void**)&circles, sizeof(Circle), GetHeapAllocator());
    ...
    OsUpdate(); // We set scaled window size, OsUpdate updates the pixel window size values for us
	...
	while (!window.should_close) {
        ...
        growing_array_init_reserve((void**)&circles_hovered, sizeof(Circle), num_circles, GetTemporaryAllocator());
		for (int i = 0; i < num_circles; i++) {
            float distance = V2Length(V2Sub(v2(mx, my), circles[i].pos));
		...
		for (int i = 0; i < growing_array_get_valid_count(circles_hovered); i++) {
            ...
            draw_circle(V2Sub(c.pos, v2(c.radius, c.radius)), v2(c.radius, c.radius), COLOR_GREEN);
		}
