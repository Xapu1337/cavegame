...
int entry(int argc, char **argv) {
	...
	while (!window.should_close) tm_scope("Frame") {
		...
		draw_text(font, tprint("Time: %f", OsGetElapsedSeconds()), font_height, v2(0, 100),  v2(1, 1), COLOR_WHITE);
		
		float now = (float)OsGetElapsedSeconds();
		...
		Vector2 justified = V2Sub(bottom_left, hello_metrics.functional_pos_min);
		
		// If we wanted to center it:
		// justified = V2Sub(justified, V2Divf(hello_metrics.functional_size, 2));
		
		draw_text(font, hello_str, font_height, justified,  v2(1, 1), COLOR_WHITE);
		
		
		local_persist bool show_bounds = false;
		if (IsKeyJustPressed('E')) show_bounds = !show_bounds;
		...
		if (n > 0) {
			...
			for (u64 i = 0; i < n; i++) {
		 	...
		 	long_text = StringConcat(long_text, gunk, GetTemporaryAllocator());
		}
		
		if (show_bounds) {
			// Visualize the bounds we get from metrics
			Gfx_Text_Metrics m = measure_text(font, long_text, font_height, v2(1, 1));
			draw_rect(V2Add(v2(-600, -200), m.visual_pos_min), m.visual_size, v4(.1, .1, .1, .2));
			draw_rect(V2Add(v2(-600, -200), m.functional_pos_min), m.functional_size, v4(1, .1, .1, .2));		
		}
