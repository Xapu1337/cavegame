...
Matrix4 make_cube_xform() {
    ...
    Vector3f32 pos = v3f32((cube.grid.x + 0.5f) * CELL_SIZE - board_half,
                           CELL_SIZE * 0.5f,
                           (cube.grid.y + 0.5f) * CELL_SIZE - board_half);

    Matrix4 m = m4_from_quat_pos(cube.orientation, pos);
    if (rotating) {
    ...
    while (!window.should_close) {
        ...
        if (rotating) {
            ...
            if (rotate_t >= 1.0f) {
                ...
                cube.orientation = quat_mul(cube.orientation,
                    quat_from_axis_angle(rotate_axis, PI32*0.5f));
                rotating = false;
                rotate_t = 0.0f;
            }
        }
